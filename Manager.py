...
class Manager():
    def __init__(self, recommendModel, attackModel, recommendArg, attackArg):
        ...
        # Shilling attack setup
        self.attackSelectSize = attackArg.selectSize

        # Target attack setup
        self.attackTarget = attackArg.attackTarget
        self.attackType = attackArg.attackType
        ...
        # Keep the rawRecommender ！！！not gracely
        self.rawRecommender = None

        # Determine target items
        if attackArg.attackTarget == "target":
        ...
        if not os.path.exists('./log/'):
            os.makedirs('./log/')
        self.logFilename = self.recommendModelName + "_" + self.attackModelName + "_" + self.datasetName + "_" + \
                           self.attackType + "_" + self.attackTarget + "_" + self.attackTargetChooseWay + "_" + \
    ...
    def RecommendTrain(self, attack=None):
        ...
        """
        if self.attackModel.recommenderGradientRequired:
            requires_grad = True
        else:
            requires_grad = False
        ...
        if attack is None:
            ...
            if self.recommendArg.load and os.path.exists(
                ...
                print("Model is existed, you need not train, loading...")
    ...
    def RecommendTest(self, attack=None):
        ...
        if attack is None:
            ...
            self.rawRecommendresult = self.recommendModel.test()
            message = "Recommender Model {} Is Trained in Clean Data".format(self.recommendModelName)
            ...
            self.attackRecommendresult = self.recommendModel.test()
            ...
            message = "Recommender Model {} Is Trained in Poison Environment No.{}".format(self.recommendModelName,
                                                                                           attack)
    ...
    def PoisonDataAttack(self):
        ...
        self.poisonDataName = self.attackModelName + "_" + self.datasetName + "_" + self.attackType + "_" + \
        ...
        self.logger.info("{} times data attack by {} has done.".format(self.times, self.attackModelName))
        self.logger.info(
            "{} new poisoning datasets is generated in {}.\n\n".format(self.times, self.poisonDatasetOutPath))

    def PoisonGradientAttack(self, attack):
        if self.rawRecommender is None:
            self.rawRecommender = deepcopy(self.recommendModel)
        self.recommendModel = deepcopy(self.rawRecommender)
        self.attackModel.gradientattack(self.recommendModel)
